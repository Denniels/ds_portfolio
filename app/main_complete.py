...
from config import WATER_QUALITY_PARAMETERS, QUALITY_CLASSIFICATION, COLORS, MAP_CONFIG, DEMO_STATIONS
from apps.modules.water_quality import get_water_quality_summary_statistics, calculate_water_quality_index
from apps.modules.chart_utils import create_temporal_chart, create_station_comparison_chart
...
class WaterQualityApp:
    ...
    def render_map_visualization(self, filters):
        """Renderiza visualización de mapa"""
        if self.filtered_data is None or not filters['stations']:
            return
            
        st.subheader("🗺️ Mapa de Estaciones de Monitoreo")
        ...
        # Agregar marcadores para estaciones con coordenadas conocidas
        for station in filters['stations']:
            if station in DEMO_STATIONS:
                coords = DEMO_STATIONS[station]
                ...
                if len(station_data) > 0:
                    popup_text = f"""
                    <b>{station}</b><br>
                    Mediciones: {len(station_data)}<br>
                    Período: {station_data['año'].min()}-{station_data['año'].max()}
                    """
        ...
        if map_data['last_object_clicked_popup']:
            st.info(f"Última estación seleccionada: {map_data['last_object_clicked_popup']}")
