...
def regenerate_model():
    ...
    # Generar features sintéticos con nombres específicos
    features = {
        'metros_totales': np.random.normal(100, 30, n_samples),
        'metros_construidos': np.random.normal(80, 25, n_samples),
    ...
    for i in range(n_samples):
        precio = (features['metros_totales'][i] * precio_base * 
                 (1 + 0.1 * features['dormitorios'][i]) * 
                 (1 + 0.15 * features['banos'][i]) * 
                 (1 + 0.05 * features['estacionamientos'][i]) *
                 (1 - 0.02 * features['antiguedad_anos'][i]))
